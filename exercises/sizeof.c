...
the sizeof operator and the min/max values defined in limits.h and float.h to display
  ...
  Size, min and max values of int, unsigned int, short int, unsigned short int, 
...
#define pti(type, size_spec, min, max) \
printf("%24s", #type": "); \
printf(C1"%zu"RESET"B, ", sizeof(type)); \
printf("range("C2 #size_spec RESET","C3 #size_spec RESET")\n", min, max)
//printf("size == "C1"%zu"RESET", min == "C3"%d"RESET", max == "C2"%d"RESET"\n", sizeof(type), min, max)
...
int main(void) {
  ...
  pti(long double, %e, LDBL_MIN, LDBL_MAX);
